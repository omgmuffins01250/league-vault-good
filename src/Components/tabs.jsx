...
export function TradingTab({
      ...
      if (Number.isFinite(targetId) && __hasAnyEntries(source)) {
        ...
        if (source instanceof Map) {
          ...
          for (const [key, team] of Object.entries(source || {})) {
            if (toInt(team?.id ?? team?.teamId ?? team?.proTeamId ?? key) === targetId) {
              pushWeeks(team);
              break;
            }
          }
  ...
  const resolveProTeamId = React.useCallback(
    ...
    [espnRostersByYear, espnRosterAcqByYear, getProTeamLookup, storeLeagueRecord]
  );
  ...
  const tradePPGExcludingBye = React.useCallback(
    (seasonKey, playerId, tradeWeek) => {
      ...
      const last = Math.min(FANTASY_PLAYOFF_END, weeksEndFromRosters(seasonKey));
  ...
  React.useEffect(() => {
    ...
    const inspectTradePPG = (seasonKey, playerId, forcedTradeWeek) => {
      ...
      const tradeWeek =
        toInt(forcedTradeWeek) ?? findTradeWeek(yr, pid) ?? 10;
