...
export function LuckIndexTab({
  ...
  // Canonicalize owner display names (matches other tabs)
  const canonicalize =
    (typeof window !== "undefined" &&
      window.__ownerMaps &&
      typeof window.__ownerMaps.canon === "function" &&
  ...
  // table axes
  const owners = (league.owners || []).map((o) => canonicalize(o));
  const seasons = league.seasonsAll || [];

  // ---------- Build starters-only totals per (year, teamId, week) ----------
  // totals[year][teamId][week] = { proj, actual }
  const teamWeekTotals = React.useMemo(() => {
    const totals = {};
    for (const [yStr, byTeam] of Object.entries(rostersByYear || {})) {
      const y = Number(yStr);
      const weekCap = Number(league?.currentWeekByYear?.[y] || 0) || 99;
      totals[y] = totals[y] || {};
      for (const [tidStr, byWeek] of Object.entries(byTeam || {})) {
        ...
        for (const [wStr, entries] of Object.entries(byWeek || {})) {
          const w = Number(wStr);
          if (!Number.isFinite(w) || w > weekCap) continue;
          let proj = 0,
            actual = 0;
          for (const e of entries || []) {
            const slotId = Number(e?.lineupSlotId ?? e?.slotId ?? e?.slot);
            if (!START_SLOTS.has(slotId)) continue; // exclude Bench (20) and IR (21)
    ...
    return totals;
  }, [rostersByYear, league?.currentWeekByYear, START_SLOTS]);

  // Robust, multi-source game extraction (like H2H)
  const getGamesFlexible = React.useCallback((leagueLike) => {
    const out = [];

    // 1) Already-normalized games
    if (Array.isArray(leagueLike?.games) && leagueLike.games.length) {
      console.log("[Luck] using league.games:", leagueLike.games.length);
      return leagueLike.games;
    }

    // 2) seasonsByYear[year].schedule[]
    const sb = leagueLike?.seasonsByYear;
    ...
    // 3) window.name seasons (fallback from earlier scrapes)
    try {
      const payload = JSON.parse(window.name || "{}");
  ...
  const games = React.useMemo(() => {
    const g = getGamesFlexible(league);
    console.log("[Luck] games loaded:", Array.isArray(g) ? g.length : 0);
    return g;
  }, [league, getGamesFlexible]);

  // ---------- COMPONENT 1: Opponent proj - opponent actual (sum over played weeks) ----------
  const comp1ByOwnerYear = React.useMemo(() => {
    const out = {};
    for (const g of games) {
      const y = Number(g?.season);
      const w = Number(g?.week);
      const t1 = Number(g?.team1?.teamId ?? g?.t1 ?? g?.team1);
      const t2 = Number(g?.team2?.teamId ?? g?.t2 ?? g?.team2);
      ...
      out[o1] ??= {};
      out[o1][y] = (out[o1][y] || 0) + d1;

      out[o2] ??= {};
      out[o2][y] = (out[o2][y] || 0) + d2;
    }

    // quick sanity: show how many owners/years we built
    if (typeof window !== "undefined") {
      const rows = Object.entries(out).map(([k, v]) => [
        k,
        Object.keys(v).length,
      ]);
      console.log("[Luck] comp1 owners/season counts:", rows);
      window.__LDEBUG_COMP1 = out;
    }
    return out;
  }, [
    games,
  ...
  // --- quick debug: confirm owner/year key matches (check 2025 or fallback 2024) ---
  React.useEffect(() => {
    const someOwner = owners?.[0];
    const someYear = seasons?.includes(2025)
      ? 2025
      : seasons?.includes(2024)
      ? 2024
      : seasons?.[seasons.length - 1];

    console.log("[Luck] sample cell", {
      someOwner,
      someYear,
      value: comp1ByOwnerYear?.[someOwner]?.[someYear],
      allOwners: Object.keys(comp1ByOwnerYear || {}),
      seasons,
    });
  }, [owners, seasons, comp1ByOwnerYear]);

  // ---------- RENDER ----------
  return (
    <div className="space-y-6">
      <Card title="Luck Components (Preview)">
        {/* Component 1 — Opponent vs Projection (sum to date) */}
        ...
        <div className="text-sm text-zinc-500 space-y-2">
          <p>
            Component 1 sums, for each manager and season, the difference
            between their
            <em> opponent’s </em> projected points and actual points using the
            opponent’s starters only (Bench/IR excluded). Positive values
            indicate “good luck” (opponents underperformed projections).
          </p>
        </div>
      </Card>
    </div>
