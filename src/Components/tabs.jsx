...
import { useOwnerNameResolver } from "../Utils/useOwnerNameResolver.js";
...
const TABLE_SHELL_WIDE_CLASS = `${TABLE_SHELL_BASE_CLASS} min-w-[640px]`;
...
const TABLE_MANAGER_CELL_CLASS =
...
export function TradingTab({
  ...
  // Data
  espnTradesDetailedBySeason = {}, // { [year]: Trade[] | {id:Trade} }
  ...
  espnRostersByYear = {}, // { [year]: { [teamId]: { [week]: RosterEntry[] } } }
  espnRosterAcqByYear = {}, // { [year]: { [teamId]: { [playerId]: {acquisitionType, name, ...} } } }
  espnWeeklyPtsByYear = {}, // { [year]: { [playerId]: Array|Object|{weeks:...} } }
  currentWeekBySeason = {}, // { [year]: number } (not used for ppg endpoint—see note)
  ...
  const buildNameIndexForYear = (year) => {
      ...
      Object.values(weeks || {}).forEach((list) => {
        safeArr(list).forEach((e) => {
          ...
          const nm =
            ...
            null;
          if (Number.isFinite(pid) && nm) idx.set(pid, nm);
        });
      });
    });

    // acquisitions-derived names
    ...
    Object.values(aq).forEach((teamMap) => {
      Object.entries(teamMap || {}).forEach(([pidRaw, meta]) => {
        ...
        const nm = meta?.name;
        if (Number.isFinite(pid) && nm && !idx.has(pid)) idx.set(pid, nm);
      });
    });
  ...
  const inferTradesByRosterDiff = (year) => {
    ...
    Object.values(acq).forEach((teamMap) => {
      Object.entries(teamMap || {}).forEach(([pidRaw, meta]) => {
        if (String(meta?.acquisitionType).toUpperCase() === "TRADE") {
          const pid = toInt(pidRaw);
          if (Number.isFinite(pid)) seeds.push(pid);
        }
      });
    });
    ...
    for (const pid of seeds) {
      const w = findTradeWeek(year, pid);
      if (!w) continue;

      const fromTeam = ownerOfAtWeek(year, pid, w - 1);
      const toTeam = ownerOfAtWeek(year, pid, w);
      if (!fromTeam || !toTeam || fromTeam === toTeam) continue;

      const Aprev = idsAt(fromTeam, w - 1);
      const Aafter = idsAt(fromTeam, w);
      const Bprev = idsAt(toTeam, w - 1);
      ...
      const trade = {
        year,
        week: w,
        leftTeamId: fromTeam, // "from" side (sending)
        rightTeamId: toTeam, // "to" side (receiving)
        leftGets: BtoA.map((id) => ({ pid: id, name: nm(id) })),
        rightGets: AtoB.map((id) => ({ pid: id, name: nm(id) })),
      };
      ...
      const R = trade.rightGets
        ...
        .sort((a, b) => a - b)
        .join(",");
      const s1 = `${trade.week}|${Math.min(
        trade.leftTeamId,
        trade.rightTeamId
      )}-${Math.max(trade.leftTeamId, trade.rightTeamId)}|${L}|${R}`;
      if (!seen.has(s1) && trade.leftGets.length && trade.rightGets.length) {
        seen.add(s1);
        out.push(trade);
      }
    }
  ...
  // ---------------- weekly points reader (robust) ----------------
  // prefer espnWeeklyPtsByYear (skips byes naturally when keys missing),
  // fallback to roster points for that week when needed.
  const ptsFromRosterWeek = (year, pid, week) => {
    const ry = espnRostersByYear?.[year] || {};
    for (const weeks of Object.values(ry)) {
      const list = weeks?.[week] || [];
  ...
  const ptsForWeek = (year, playerId, week) => {
    const map = espnWeeklyPtsByYear?.[year];
    const row = map?.[playerId];
    const coerce = (v) => (Number.isFinite(Number(v)) ? Number(v) : null);

    if (row != null) {
      // A) flat number[] indexed by (week-1)
      if (Array.isArray(row)) {
        const ix = week - 1;
        if (ix >= 0 && ix < row.length) return coerce(row[ix]);
      }
      // B) {weeks:{...}} or plain object keyed by week number/string or "W10"
      const base =
      ...
      if (base && typeof base === "object") {
        ...
        for (const v of Object.values(base)) {
          if (
            v &&
            typeof v === "object" &&
            (v.week === week || v.scoringPeriodId === week)
          ) {
            const tries = [
              "pts",
              "points",
  ...
  // --------- normalize detailed trade shapes (Kona/ESPN) ----------
  const normalizeTrade = (year, tr) => {
    if (!tr || typeof tr !== "object") return null;

    const week =
      toInt(tr.week) ??
      toInt(tr.matchupPeriodId) ??
      toInt(tr.scoringPeriodId) ??
      toInt(tr.period) ??
      null;

    const txId = tr.relatedTransactionId || tr.id || null; // capture for dedupe

    // A) { teams:[ {teamId, receives:[{playerId,name}]}, ... ] }
    if (Array.isArray(tr.teams) && tr.teams.length >= 2) {
      const a = tr.teams[0] || {};
      const b = tr.teams[1] || {};
      const getPkg = (x) =>
        safeArr(x.receives || x.adds || x.to || x.in).map((p) => {
          const pid = toInt(p?.playerId ?? p?.id);
          const nm =
            p?.name ||
            p?.fullName ||
      ...
      return {
        ...
        week,
        txId,
        leftTeamId: toInt(a.teamId),
        rightTeamId: toInt(b.teamId),
        leftGets: getPkg(a),
        rightGets: getPkg(b),
      };
    }

    // B) { a:{teamId, in:[...]}, b:{teamId, in:[...]} }
    if (tr.a || tr.b) {
      const A = tr.a || {};
      const B = tr.b || {};
      const take = (node) =>
        safeArr(node.in || node.receives || node.adds).map((p) => {
          const pid = toInt(p?.playerId ?? p?.id);
          const nm =
            p?.name ||
            p?.fullName ||
    ...
    if (tr.leftGets || tr.rightGets) {
      ...
      return {
        ...
        week,
        txId,
        leftTeamId: toInt(tr.leftTeamId),
        rightTeamId: toInt(tr.rightTeamId),
        leftGets: toPkg(tr.leftGets),
        rightGets: toPkg(tr.rightGets),
      };
    }

    // D) ESPN/Kona: items[{type:'TRADE', playerId, fromTeamId, toTeamId}], teamActions
    if (Array.isArray(tr.items)) {
      ...
      return {
        ...
        leftTeamId: toInt(A),
        rightTeamId: toInt(B),
        leftGets: toPkg((i) => i?.toTeamId === A),
        rightGets: toPkg((i) => i?.toTeamId === B),
      };
    }
  ...
  // ---------------- build trades by year (merge detailed+inferred, dedupe) ----------------
  const byYear = {};
  yearList.forEach((yr) => {
    // detailed
    ...
    const detBucket = new Map();
    detailed.forEach((t, i) => {
      const wk = toInt(t?.week, -1);
      const k = `${wk}|${canonKeyTeams(t.leftTeamId, t.rightTeamId)}`;
      if (!detBucket.has(k)) detBucket.set(k, []);
      detBucket.get(k).push({ idx: i, t });
    });
    ...
    const unionPlayers = (A = [], B = []) => {
      ...
      [...A, ...B].forEach((p) => {
        ...
        if (!map.has(id)) map.set(id, { pid: id, name: p?.name });
      });
      return Array.from(map.values());
    };

    // Merge inferred into matching detailed trade (same week & teams; at least one overlapping pid)
    const consumedInf = new Set();
    inferred.forEach((inf) => {
      const wk = toInt(inf?.week, -1);
      const k = `${wk}|${canonKeyTeams(inf.leftTeamId, inf.rightTeamId)}`;
      const cands = detBucket.get(k);
      if (!cands || !cands.length) return;

      // pick the detailed trade with the largest overlap
      let best = null,
        bestOverlap = 0,
        bestSwap = false;
      cands.forEach(({ t }) => {
        const leftOverlap = t.leftGets.filter((lp) =>
          inf.leftGets.some((ip) => +ip.pid === +lp.pid)
        ).length;
        const rightOverlap = t.rightGets.filter((rp) =>
          inf.rightGets.some((ip) => +ip.pid === +rp.pid)
        ).length;
        const same = leftOverlap + rightOverlap;

        const swapLeft = t.leftGets.filter((lp) =>
          inf.rightGets.some((ip) => +ip.pid === +lp.pid)
        ).length;
        const swapRight = t.rightGets.filter((rp) =>
          inf.leftGets.some((ip) => +ip.pid === +rp.pid)
        ).length;
        const swapped = swapLeft + swapRight;
      ...
      const det = best;
      const addL = bestSwap ? inf.rightGets : inf.leftGets;
      const addR = bestSwap ? inf.leftGets : inf.rightGets;

      det.leftGets = unionPlayers(det.leftGets, addL);
      det.rightGets = unionPlayers(det.rightGets, addR);
      consumedInf.add(inf);
    });

    // Strong de-dupe:
    // 1) If txId present on detailed, collapse all with same txId.
    // 2) Otherwise collapse by canonical trade signature.
    const out = [];
    ...
    const push = (t) => {
      if (!t) return;
      // canonicalize empty sides are not allowed
      if (!(t.leftGets?.length && t.rightGets?.length)) return;

      const txId = t.txId && String(t.txId);
      if (txId) {
    ...
    detailed.forEach(push);
    inferred.filter((t) => !consumedInf.has(t)).forEach(push);
...
export function RosterTab({
  ...
  const { ownerNameOf, normalizeOwnerNameLoose } = useOwnerNameResolver({
    league,
  });
  ...
  const hidden = React.useMemo(() => {
    const list = [];
    if (Array.isArray(league?.hiddenManagers)) {
      list.push(...league.hiddenManagers);
    }
    if (Array.isArray(hiddenManagers)) {
      list.push(...hiddenManagers);
    }
    const set = new Set();
    list.forEach((name) => {
      if (!name) return;
      ...
      if (trimmed) set.add(trimmed);
      const normalized =
        typeof normalizeOwnerNameLoose === "function"
          ? normalizeOwnerNameLoose(trimmed) || normalizeOwnerNameLoose(name)
          : null;
      if (normalized) set.add(normalized);
    });
    return set;
  }, [league?.hiddenManagers, hiddenManagers, normalizeOwnerNameLoose]);
  ...
  const resolveOwner = useCallback(
    (teamId) => {
      ...
      let resolved =
        Number.isFinite(numeric) && typeof ownerNameOf === "function"
          ? ownerNameOf(season, numeric)
          : null;
      ...
      const fallbackLabel =
        Number.isFinite(numeric) && numeric > 0
          ? `Team ${numeric}`
          : resolved || (teamId != null ? String(teamId).trim() : "Team");
      const base =
        (resolved &&
          typeof normalizeOwnerNameLoose === "function" &&
          normalizeOwnerNameLoose(resolved)) ||
        (typeof resolved === "string" && resolved.trim()) ||
        (typeof normalizeOwnerNameLoose === "function"
          ? normalizeOwnerNameLoose(fallbackLabel)
          : null) ||
        fallbackLabel ||
        "Team";
    ...
    [season, ownerNameOf, normalizeOwnerNameLoose, mergeMap]
  );
  ...
  const teamIds = React.useMemo(
    () =>
      Object.keys(byTeam)
        ...
        .filter((tid) => {
          ...
          const canonical =
            typeof normalizeOwnerNameLoose === "function"
              ? normalizeOwnerNameLoose(name) || name
              : name;
    ...
    [byTeam, resolveOwner, hidden, normalizeOwnerNameLoose]
  );
...
export function DraftTab({
  ...
  const flatDraftRows = React.useMemo(
    () =>
      Object.values(draftByYear || {}).flatMap((rows) =>
  ...
  const normalizeTeamLabel = React.useCallback(
    (value) =>
      String(value || "")
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, "")
  ...
  const ownerMapsBySeason = React.useMemo(() => {
    const map = new Map();
    const ensureSeason = (seasonNum) => {
      if (!map.has(seasonNum)) {
        map.set(seasonNum, { byTeam: new Map(), byOwner: new Map() });
      }
      return map.get(seasonNum);
    };
    const insert = (seasonNum, teamId, ownerName) => {
      if (!Number.isFinite(seasonNum) || !Number.isFinite(teamId)) return;
      const normalized = canonicalize(ownerName);
      if (!normalized) return;
      const bucket = ensureSeason(seasonNum);
      if (!bucket.byTeam.has(teamId)) bucket.byTeam.set(teamId, normalized);
      if (!bucket.byOwner.has(normalized)) bucket.byOwner.set(normalized, teamId);
    };
    const mergeSource = (source) => {
      Object.entries(source || {}).forEach(([seasonKey, ownerMap]) => {
        const seasonNum = Number(seasonKey);
        if (!Number.isFinite(seasonNum)) return;
        Object.entries(ownerMap || {}).forEach(([teamKey, ownerValue]) => {
          const teamId = Number(teamKey);
          insert(seasonNum, teamId, ownerValue);
        });
      });
    };
    ...
    if (typeof window !== "undefined") {
      ...
      mergeSource(S.ownerByTeamByYear);
      mergeSource(S.espnOwnerNameByTeamByYear);
      mergeSource(S.espnOwnerByTeamByYear);
    }
    ...
    Object.entries(managerBySeasonTeam || {}).forEach(([seasonKey, byTeam]) => {
      ...
      Object.entries(byTeam || {}).forEach(([teamKey, ownerName]) => {
        const teamId = Number(teamKey);
        insert(seasonNum, teamId, ownerName);
      });
    });

    return map;
  }, [league?.ownerByTeamByYear, managerBySeasonTeam, canonicalize]);

  const teamNameToOwnerByYear = React.useMemo(() => {
    const map = new Map();
    const ensureSeason = (seasonNum) => {
      if (!map.has(seasonNum)) {
        map.set(seasonNum, { exact: new Map(), normalized: new Map() });
      }
      return map.get(seasonNum);
    };
    const addEntry = (seasonNum, teamName, ownerName) => {
      if (!Number.isFinite(seasonNum)) return;
      const label = String(teamName || "").trim();
      if (!label) return;
      const normalizedOwner = canonicalize(ownerName);
      if (!normalizedOwner) return;
      const bucket = ensureSeason(seasonNum);
      if (!bucket.exact.has(label)) bucket.exact.set(label, normalizedOwner);
      const normalizedLabel = normalizeTeamLabel(label);
      if (normalizedLabel && !bucket.normalized.has(normalizedLabel)) {
        bucket.normalized.set(normalizedLabel, normalizedOwner);
      }
    };
    const addFromSource = (source) => {
      Object.entries(source || {}).forEach(([ownerName, byYear]) => {
        Object.entries(byYear || {}).forEach(([seasonKey, teamName]) => {
          const seasonNum = Number(seasonKey);
          addEntry(seasonNum, teamName, ownerName);
        });
      });
    };

    addFromSource(league?.teamNamesByOwner);

    if (typeof window !== "undefined") {
      const S = window.__sources || {};
      addFromSource(S.teamNamesByOwner);
      addFromSource(S.espnTeamNamesByOwner);
    }

    ownerMapsBySeason.forEach((bucket, seasonNum) => {
      bucket.byTeam.forEach((ownerName, teamId) => {
        addEntry(seasonNum, `Team ${teamId}`, ownerName);
      });
    });
  ...
  const ownerNameResolver = React.useCallback(
    (row, yr) => {
      if (!row) return "—";
      ...
      const tryResolveByTeamId = (teamIdRaw) => {
        const teamId = Number(teamIdRaw);
        if (!Number.isFinite(teamId)) return null;
        if (season != null && typeof ownerNameOf === "function") {
          const resolved = ownerNameOf(season, teamId);
          if (resolved) return canonicalize(resolved);
        }
        if (season != null) {
          const bucket = ownerMapsBySeason.get(season);
          if (bucket?.byTeam?.has(teamId)) {
            return bucket.byTeam.get(teamId);
          }
        }
        if (typeof resolveAliasForTeamId === "function") {
          const alias = resolveAliasForTeamId(teamId);
          if (alias) return canonicalize(alias);
        }
        const aliasLabel = canonicalize(`Team ${teamId}`);
        if (aliasLabel && aliasLabel !== `Team ${teamId}`) {
          return aliasLabel;
        }
        if (season == null) {
          for (const bucket of ownerMapsBySeason.values()) {
            if (bucket.byTeam.has(teamId)) {
              return bucket.byTeam.get(teamId);
            }
          }
      ...
      for (const cand of teamIdCandidates) {
        const resolved = tryResolveByTeamId(cand);
        if (resolved) return resolved;
      }
      ...
      if (teamNameCandidates.length) {
        const seasonsToSearch = [];
        if (season != null) seasonsToSearch.push(season);
        ownerMapsBySeason.forEach((_, seasonKey) => {
          if (!seasonsToSearch.includes(seasonKey)) {
            seasonsToSearch.push(seasonKey);
          }
        });

        for (const seasonKey of seasonsToSearch) {
          const bucket = teamNameToOwnerByYear.get(seasonKey);
          if (!bucket) continue;
          for (const candidate of teamNameCandidates) {
            const exact = bucket.exact.get(candidate);
            if (exact) return exact;
            const normalizedLabel = normalizeTeamLabel(candidate);
            if (normalizedLabel) {
              const match = bucket.normalized.get(normalizedLabel);
    ...
    [
      ...
      ownerNameOf,
      resolveAliasForTeamId,
      teamNameToOwnerByYear,
    ]
  );

  const hiddenSources = React.useMemo(() => {
    const list = [];
    if (Array.isArray(league?.hiddenManagers)) list.push(...league.hiddenManagers);
    if (Array.isArray(hiddenManagers)) list.push(...hiddenManagers);
    return list;
  }, [league?.hiddenManagers, hiddenManagers]);

  const hiddenSet = React.useMemo(() => {
    const set = new Set();
    hiddenSources.forEach((name) => {
      const canonical = canonicalize(name);
      if (canonical) set.add(canonical);
    });
    return set;
  }, [hiddenSources, canonicalize]);
  ...
  const hiddenNormSet = React.useMemo(() => {
    ...
    return set;
  }, [hiddenSources, normName]);

  function groupByOwner(rows = []) {
    const m = new Map();
    rows.forEach((r) => {
      const resolved =
        ownerNameResolver(r, r?.year ?? r?.season ?? year) || "—";
      const owner = canonicalize(resolved) || "—";
    ...
    for (const yrStr of Object.keys(draftByYear || {})) {
      ...
      for (const r of yrRows) {
        const raw =
          ...
          r?.owner_full ??
          "";
        const display = ownerDisplay(raw);
        const ownerCanonical = canonicalize(display) || display || "—";
        const ownerNorm = normName(ownerCanonical);
        const isHidden =
          hiddenSet.has(ownerCanonical) || hiddenNormSet.has(ownerNorm);
        if (isHidden) continue; // skip hidden
        if (!map.has(ownerCanonical)) map.set(ownerCanonical, new Set());
        map.get(ownerCanonical).add(Number(yrStr));
      }
    }
  ...
  const draftedPoints = React.useMemo(() => {
    ...
    for (const [yrStr, picks] of Object.entries(draftByYear || {})) {
      ...
      (picks || []).forEach((r) => {
        ...
        const resolvedOwner = ownerNameResolver(r, yr) || "—";
        const owner = canonicalize(resolvedOwner) || "—";
        const ownerNorm = normName(owner);
    ...
    const teamIdForOwner = (yr, owner) => {
      const seasonNum = Number(yr);
      if (!Number.isFinite(seasonNum)) return null;
      const canonicalOwner = canonicalize(owner);
      if (!canonicalOwner) return null;
      const bucket = ownerMapsBySeason.get(seasonNum);
      if (bucket?.byOwner?.has(canonicalOwner)) {
        return bucket.byOwner.get(canonicalOwner);
      }
      const aliasMatch = canonicalOwner.match(/^team\s*(\d+)$/i);
      if (aliasMatch) {
    ...
    for (const yrStr of Object.keys(draftByYear || {})) {
      ...
      (draftByYear?.[yr] || []).forEach((r) => {
        const resolved = ownerNameResolver(r, yr) || "—";
        const owner = canonicalize(resolved) || "—";
        const ownerNorm = normName(owner);
        if (hiddenSet.has(owner) || hiddenNormSet.has(ownerNorm)) return;
        ownersThisYear.add(owner);
      });
    ...
    return { perSeason, perWeek, totals: totalsRows };
  }, [
    draftByYear,
    includeKeepers,
    hiddenSet,
  ...
  const openOwnerBreakdownForYear = React.useCallback(
    (owner) => {
      const allRows = (draftWithFinish?.[year] || []).filter((r) => {
        const resolved = ownerNameResolver(r, year);
        return canonicalize(String(resolved)) === canonicalize(String(owner));
      });
      const items = [];
    ...
    [draftWithFinish, year, ownerNameResolver, ownerDisplay, contribForPick]
  );
  ...
  const openOwnerBreakdownAllYears = React.useCallback(
    (owner) => {
      ...
      for (const yrStr of Object.keys(draftWithFinish || {})) {
        ...
        for (const r of draftWithFinish[yrStr] || []) {
          const resolved = ownerNameResolver(r, yr);
          if (canonicalize(String(resolved)) !== canonicalize(String(owner)))
            continue;
          const c = contribForPick(r);
          if (!c) continue;
          items.push({
    ...
    [draftWithFinish, ownerNameResolver, ownerDisplay, contribForPick]
  );
...
export function WeeklyOutlookTab({
  ...
  const playersOnByeByOwner = React.useMemo(() => {
    ...
    const byTeam = rostersByYear?.[yr] || rostersByYear?.[String(yr)] || {};
    ...
    const resolveOwnerName = (teamId) => {
      const tid = Number(teamId);
      if (!Number.isFinite(tid)) return null;
      const fromMap = ownerNameFromMap ? ownerNameFromMap(yr, tid) : null;
      const fallback = ownerByTeam?.[tid] || ownerByTeam?.[String(tid)] || null;
    ...
    const entryName = (entry) => {
      ...
      const combo = [entry?.player?.firstName, entry?.player?.lastName]
        .filter(Boolean)
        .join(" ");
  ...
  const largestProjectionGap = React.useMemo(() => {
    ...
    matchupsThisWeek.forEach((m) => {
      ...
      if (
        ...
        diff > best.diff ||
        (diff === best.diff &&
          pairKeyOf(m.aName, m.bName).localeCompare(best.key) < 0)
      ) {
        best = {
    ...
    byGame.forEach(({ week, pairKey, rows }) => {
      ...
      const scoreFor = (owner) => {
        for (const row of rows) {
          if (row.owner === owner && Number.isFinite(row.pf))
            return Number(row.pf);
          if (row.opp === owner && Number.isFinite(row.pa))
            return Number(row.pa);
        }
        return null;
      };
            ...
            {matchupsThisWeek.map((m, i) => {
              ...
              if (standingsA?.rank && standingsB?.rank) {
                ...
                if (ranks[0] === 1 && ranks[1] === 2) {
                  ...
                  const topDogSecond =
                    topDogFirst === m.aName ? m.bName : m.aName;
                  pushFact(
                    `topdog-${pairKey}`,
...
export function LuckIndexTab({
  ...
  function getGamesFlexible(league) {
    ...
    const extractTeamInfo = (
      ...
      if (side && typeof side === "object") {
        ...
        if (side.projectedPointsByScoringPeriod && week != null) {
          projectedCandidates.push(
        ...
        if (side.__proj != null) {
          if (typeof side.__proj === "number") {
            projectedCandidates.push(side.__proj);
          } else if (
            projectedKey &&
            typeof side.__proj === "object" &&
            side.__proj[projectedKey] != null
          ) {
            projectedCandidates.push(side.__proj[projectedKey]);
          }
        }
      }
      projectedCandidates.push(...extraProjected);
      let projected = null;
      for (const cand of projectedCandidates) {
        const n = Number(cand);
      ...
      if (side && typeof side === "object") {
        ...
        if (week != null) {
          scoreCandidates.push(
            side.pointsByScoringPeriod?.[week],
            side.totalPointsByScoringPeriod?.[week]
          );
        }
      }
      scoreCandidates.push(...extraScores);
      let score = 0;
      ...
      return {
        ...
        projected: projected != null ? projected : null,
        score,
      };
    };

    const buildGameRecord = ({
      season,
      week,
      home,
      away,
      ...
      const wk = Number(week);
      const yr = Number(season);
      if (!Number.isFinite(yr) || !Number.isFinite(wk) || wk <= 0) return null;

      const team1 = extractTeamInfo(home, {
        week: wk,
        projectedKey: "home",
        fallbackIds: fallbackIds.home || [],
        fallbackOwners: fallbackOwners.home || [],
        extraProjected: extraProjected.home || [],
        extraScores: extraScores.home || [],
      });
      const team2 = extractTeamInfo(away, {
        week: wk,
        projectedKey: "away",
        fallbackIds: fallbackIds.away || [],
        fallbackOwners: fallbackOwners.away || [],
        extraProjected: extraProjected.away || [],
        extraScores: extraScores.away || [],
      });

      if (!team1.teamId && !team1.ownerName) return null;
      if (!team2.teamId && !team2.ownerName) return null;
      ...
      return {
        ...
        team1: {
          ...
          score: Number.isFinite(team1.score) ? team1.score : 0,
          projected:
            team1.projected != null && Number.isFinite(team1.projected)
              ? team1.projected
              : null,
        },
        team2: {
          ...
          score: Number.isFinite(team2.score) ? team2.score : 0,
          projected:
            team2.projected != null && Number.isFinite(team2.projected)
              ? team2.projected
              : null,
        },
        is_playoff: Boolean(isPlayoff),
      };
    };
    ...
    if (Array.isArray(league?.games) && league.games.length) {
      ...
      for (const g of league.games) {
        ...
        const record = buildGameRecord({
          ...
          fallbackIds: {
            home: [g?.team1Id, g?.homeTeamId, g?.teamId1, g?.homeId],
            away: [g?.team2Id, g?.awayTeamId, g?.teamId2, g?.awayId],
          },
          fallbackOwners: {
            home: [
              g?.team1Owner,
          ...
          extraProjected: {
            home: [g?.team1Projected, g?.homeProjected],
            away: [g?.team2Projected, g?.awayProjected],
          },
          extraScores: {
            home: [
              g?.team1Score,
    ...
    const scheduleSources = [
      {
        label: "league.seasonsByYear.schedule",
        data: league?.seasonsByYear,
        extractor: (seasonObj) => seasonObj?.schedule || [],
      },
      {
        label: "league.scheduleByYear",
        data: league?.scheduleByYear,
        extractor: (arr) => arr || [],
      },
      {
        label: "league.espnScheduleByYear",
        data: league?.espnScheduleByYear,
        extractor: (arr) => arr || [],
      },
    ];
    ...
    for (const source of scheduleSources) {
      ...
      const normalized = [];
      for (const [yStr, seasonData] of Object.entries(source.data || {})) {
        const seasonNum = Number(yStr);
        if (!Number.isFinite(seasonNum)) continue;
        const gamesArr = source.extractor(seasonData) || [];
        for (const m of gamesArr) {
          const week =
            m?.matchupPeriodId ??
            m?.scoringPeriodId ??
            m?.week ??
            ...
            m?.matchupPeriod ??
            0;
          const home = m?.home ?? m?.homeTeam ?? m?.team1 ?? m?.teamOne ?? {};
          const away = m?.away ?? m?.awayTeam ?? m?.team2 ?? m?.teamTwo ?? {};
          ...
          const record = buildGameRecord({
            ...
            isPlayoff:
              m?.playoffTierType && m.playoffTierType !== "NONE"
                ? true
                : m?.isPlayoff ?? m?.is_playoff ?? false,
            fallbackIds: {
              home: [
                m?.homeTeamId,
                m?.team1Id,
      ...
      if (normalized.length) {
        console.log(`[Luck] using ${source.label}:`, normalized.length);
        return normalized;
      }
    }
    ...
    try {
      ...
      const normalized = [];
      for (const s of seasons) {
        const seasonNum = Number(s?.seasonId);
        if (!Number.isFinite(seasonNum)) continue;
          ...
          const record = buildGameRecord({
            ...
            isPlayoff:
              m?.playoffTierType && m.playoffTierType !== "NONE"
                ...
                : m?.isPlayoff ?? m?.is_playoff ?? false,
            fallbackOwners: {
              home: [home?.ownerName, m?.homeOwner],
              away: [away?.ownerName, m?.awayOwner],
            },
            extraScores: {
      ...
      if (normalized.length) {
        console.log("[Luck] using window.name seasons:", normalized.length);
        return normalized;
      }
    } catch {}

    console.warn(
      "[Luck] No games found. Expect league schedule data via seasonsByYear/scheduleByYear"
    );
    return [];
  }
  ...
  const comp5WeightedTotal = React.useMemo(() => {
    ...
    const tableShellLuck = `${TABLE_SHELL_BASE_CLASS} min-w-[420px]`;
    const tableBodyClass = TABLE_BODY_CLASS;
    const headRowClass = TABLE_HEAD_ROW_CLASS;
    const placeCellClass =
      "px-4 py-3 text-left align-middle text-slate-800 dark:text-slate-100";
    const managerCellClass = TABLE_MANAGER_CELL_CLASS;
    const valueCellClass = TABLE_VALUE_CELL_CLASS;
    const valueButtonClass = TABLE_VALUE_BUTTON_CLASS;
    const toggleButtonBase = TOGGLE_BUTTON_BASE_CLASS;
    const toggleButtonActive = TOGGLE_BUTTON_ACTIVE_CLASS;
    const toggleButtonInactive = TOGGLE_BUTTON_INACTIVE_CLASS;
    const softButtonClass = SOFT_BUTTON_CLASS;
  return (
    <div className="space-y-6">
